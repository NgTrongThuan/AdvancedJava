...
public class CDController {
	...
	public void addCD(CD obj) throws ClassNotFoundException, SQLException {
		try (Connection conn = db.connect()) {
			String sql = "INSERT INTO cd VALUES(NULL, '" + obj.getName()
					+ "', '" + obj.getSinger() + "', " + obj.getNumOfSong()
					+ ", " + obj.getPrice() + ")";
	...
	public void runAddCD() throws IOException, ClassNotFoundException, SQLException {
		...
		if(numbersong <= 0) throw new ArithmeticException("Number song is not valid!");
		if(price <= 0) throw new ArithmeticException("Price is not valid!");
	...
	public static List<CD> searchCD(String name) throws ClassNotFoundException,
			SQLException {
		List<CD> rs;
		try (Connection conn = db.connect()) {
			rs = new ArrayList<>();
	...
	public void runSearchCD() throws IOException, SQLException,
	...
	public void updateCD(CD cd) throws ClassNotFoundException,
			SQLException {
		try (Connection conn = db.connect()) {
			String sql = "UPDATE cd set numbersongs = " + cd.getNumOfSong()
	...
	public void deleteCD(CD cd) throws ClassNotFoundException,
			SQLException {
		try (Connection conn = db.connect()) {
	...
	public void runDeleteCD() throws ClassNotFoundException, SQLException, NumberFormatException, IOException{
		...
		int id = Integer.parseInt(input.readLine());
		if(id <= 0) throw new ArithmeticException("ID is not valid!");
		CD cd = this.getCD(lst, id);
